...
*Topic*: An alternative perspective on the logic of anaphora and information flow - /Externally Dynamic Dynamic Semantics/ - which builds on Charlow's insight (today) that an austere, Montagovian grammar can be systematically lifted into a dynamic setting..

- *Obligatory reading*: My manuscript "Towards a principled logic of anaphora" [cite:@Elliott2020g].
- Optional related readings:
  + Matt Mandelkern's L&P paper on pseudo-dynamics [cite:@Mandelkern2022].
  + Matthew Gotham's AC paper on negation and disjunction in DPL [cite:@Gotham2019].
  + Daniel Rothschild's AC paper on trivalence and dynamic semantics [cite:@Rothschild2017].
    
** Today's goals


- Recap of DPL.
- A closer examination of the DPL theory of donkey anaphora.
- A detailed assessment of the empirical coverage of DPL (and related theories), and where they fail.
- Incorporating the insights of DPL into FCS ("pointwise FCS").
- A re-assessment of the dynamic slogan - do we really need CCPs?
- Some speculations on familiarity and uniqueness.
- Charlow's monadic approach to compositional dynamic semantics.
...
I'll tentatively suggest that we might want to try to derive strong readings via independently motivated pragmatic strengthening mechanisms developed for deriving free choice [cite:@Bar-Lev2018;@Bar-LevFox2017].
...
We keep the DPL semantics for existentials (already implicit in our presentation of partial DPL), but incorporate a novelty check.[fn:5]
...
A tentative suggestion: in a setting with CCPs, we can minimally refactor the semantics of indefinites such that they create singleton /sub-contexts/; definite descriptions require uniqueness relative to the input context, but when uniqueness is assessed relative to a sub-context, the requirement dissipates. 
...
[fn:5] Note that this avoids the so-called "destructive update" problem of DPL (see also [cite:@Dekker1996;@Charlow2019d]).
