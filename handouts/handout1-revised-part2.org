...
- Every world \(w \in F_5\) is mapped to the set of all assignments. 
...
#+begin_tcolorbox
Compute the satisfaction sets of each of \(w_1\), \(w_2\) and \(w_{12}\) at \(F\), assuming that \(x\) is the only variable.
#+end_tcolorbox
...
These facts suggest that disjoint reference should be derived as a /primary implicature/ (in the sense of [cite:@Sauerland2004b]), i.e., the speaker is not certain that \(x\) and \(y\) are mapped to same individual. 
...
We can easily formalize /contextual informativity/ in FCS as follows:
...
- A stronger, secondary implicature could be derived pragmatically [cite:@Sauerland2004b], or via an exhaustification operator geared to act on CCPs [cite:@Fox2007a].
...
After all, there's a reason people believed that /uniqueness/ is entailed by definite descriptions (the examples here are a variant of data discussed by [cite:@MandelkernRothschild2020a]).
...
- As it stands however, it is mysterious why definites require uniqueness when they don't have an antecedent. As emphasized by [cite:@MandelkernRothschild2020a], this is arguably a question of /accommodation/ - in fact, the logic is similar to the proviso problem.
