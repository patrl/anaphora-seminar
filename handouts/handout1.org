...
** Quantification and donkey anaphora 
...
Now, the meaning of /every/ (it's complicated).

\[\mathbf{every}(p)(q) := \lambda F\,.\,\Set{(w,g)\in F|\begin{aligned}[c]
&\set{g'|g' \sim_F g \wedge (w,g') \in p(F)}\\
&\subseteq \set{g'|\exists g''[(w,g'') \in q(p(F)) \wedge  g'' \sim_{p(F)} g']}
\end{aligned}}\]
...
- Now, for each world-assignment pair \((w,g) \in F\), run the following test: 
  * gather up the assignments \(g'\), such that \((w,g')\) in \(F'\), and \(g' \sim_F g\). Return the /restrictor set/.
  * gather up those assignments \(g'\), s.t. you can find some \((w,g'')\) in \(F''\), where \(g' \sim_F' g''\). Return the /scope set/.
  * Check that the /restrictor set/ is a subset of the /scope set/.
...
Tentatively updating \(c_\top \) with /a\(^x\) cat/ will give us the initial context minus the world assignment pairs where \(x\) gets mapped to a dog.

- \(F' = c_\top - \set{(w_a,[x \to b, y \to a]),(w_a,[x \to b, y \to b])}\)
  
Tentatively updating \(F'\) with /\(t_x\) died/ will give us \(F'\) minus (i) the world-assignment pairs where no cats died, and (ii) the world assignment pairs where \(x\) gets mapped to cat that didn't die. 

- \(F'' = F' - \set{(w_\emptyset,*),(w_b,[x \to a, y \to a]),(w_b,[x \to a,y \to b])}\)
  
Now we run through the test, for each world assignment pair in \(c_\top\). This will take a long time, but let's just give some illustrative examples.

Let's say we start with an arbitrary pairing \((w_\emptyset,g)\).

 - \(F'\) doesn't eliminate any world assignment pairs \((w_\emptyset,*)\), so the restrictor set is just every possible assignment.  
 - \(F''\) eliminates /all/ world assignment pairs \((w_\emptyset,*)\), so trivially, the scope set is empty, and our test fails.
 - All world-assignment pairs \((w_\emptyset ,*)\) are therefore eliminated from the output set, as desired.
   
Let's take an arbitrary pairing \((w_b,g)\), and perform our test.

- Again, \(F'\) doesn't eliminate any world assignment pairs \((w_b,*)\), so the restrictor set is just every possible assignment.
- \(F''\) eliminates \((w_b,g)\) pairs where \(g_x = a\), so we're left with \((w_b,[x \to b, y \to a]),(w_b,[x \to b,y \to b])\). We now check, for every possible assignment, whether either \([x \to b, y \to a]\) or \([x \to b,y \to b]\) assign the same value to \(x\), which is familiar in \(F'\). This will fail for assignments mapping \(x\) to \(a\), and so the test fails.
- As desired, all world-assignment pairs \((w_b,*)\) are eliminated from the output set.
  
Now, what about a world in where the test should pass: \(w_a\). Let's first consider a pairing \((w_a,g)\), where \(g\) maps \(x\) to a \(a\).
  
Now let's consider a pairing \((w_a,g)\), where \(g\) maps \(x\) to \(b\).

*** TODO keep going with the example

*** TODO mention proportion problem 

*** TODO mention weak and strong readings
 
*** TODO the equivalence relation is inducing a partition
...
#+name: ex:donkey
#+begin_exe
\ex Every philosopher who had a\(^x\) question asked it\(_x\).
#+end_exe

Heim takes the desired truth-conditions of this sentence to be that /every philosopher asked every question that they had/.

How do we derive this compositionally? Heim suggests treating /every/ as an operator on CCPs, just like the indefinite article.
