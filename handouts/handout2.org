...
Topic: dynamic semantics as a theory of composition.
...
** Informativity and implicature in FCS
...
We haven't said anything about definite descriptions, but FCS very naturally accounts for definite descriptions in the following way, e.g., for "the_x woman sat down.":

#+name: ex:definite
#+begin_exe
\ex \(\lambda c\,.\,\begin{cases}
\set{(w,g) \in c\,.\,\mathbf{walkedIn}_w(g_x)}&\forall(w,g) \in c[\mathbf{woman}_w(g_x)]\\
...
We can account for \(\phi \)-features in a parallel fashion. A putative CCP for "She_x sat down":
...
#+name: date
#+begin_exe
\ex John went on a date; he liked the_x woman, so he planned to see again.
#+end_exe

#+name: date2
#+begin_exe
\ex John went on dates with a few different women; he liked the_x woman, so he planned to see her again.
#+end_exe
...
- As it stands however, it is mysterious why definites require uniqueness when they don't have an antecedent. As emphasized by [cite:@MandelkernRothschild2020], this is a problem for theories of /accommodation/ - in fact, the logic is very similar to the proviso problem.
...
* TODO say something about false files (this relates to triviality)
* Partial FCS
...
* TODO check if the partial pointwise presentation is correct, especially for the bridge principle
