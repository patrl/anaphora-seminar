...
#+begin_exe
\ex Andreea might have a\(^v\) spouse.
\ex She's married.
\ex Have you met him\(_v\)?
#+end_exe

#+name: modalrothschild2
#+begin_exe
\ex Andreea might have a\(^v\) spouse. If she's indeed married, I'll confront him\(_v\).
#+end_exe

What this seems to indicate is that, when uttered against \(c\), "might \(\phi \)" allows \(\phi \) to introduce anaphoric information /only relative to the worlds in \(c\) at which \(\phi \) is true/.

#+name: epistemic-example
#+begin_exe
\ex \(c[\Diamond\exists_vS(v)]^+ = \begin{cases}
\set{(w,h)|(w,g) \in c \wedge g[v]h \wedge h_v \in I_w(S)} \cup \set{(w,g) \in c|I_w(S) = \emptyset }&\exists w \in c_w[I_w(S) \neq \emptyset]\\
\emptyset &\text{otherwise} 
\end{cases}\)
#+end_exe

** Existential projection for epistemic modals

#+name: mightmight
#+begin_exe
\ex There might be a bathroom, and it might be upstairs.
\ex If it's possible there's a bathroom, then it's possible that it's upstairs.
\ex Either it's impossible there's a bathroom, or it's possible that it's upstairs.
\ex Either there mustn't be a bathroom, or it's possible it's upstairs.
#+end_exe

Generalization: presuppositions project existentially through epistemic modals.

A new semantics for epistemic modals which captures this:

#+name: epistem-proj
#+begin_exe
\ex \(c[\Diamond \phi]^+ = c\text{ if }\exists c' \subseteq c[c'[\phi]^+ \neq \emptyset]\text{ else }\emptyset  \)
#+end_exe

An epistemic modal requires that there be /some sub-part/ of \(c\) against which \(\phi \) is satisfied.
