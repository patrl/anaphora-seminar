...
#+name: poss-bath
#+begin_exe
\ex There might be a\(^v\) bathroom.<<poss-bath1>>
\ex It\(_v\) might be upstairs.<<poss-bath2>>
#+end_exe

([[poss-bath1]]-[[poss-bath2]]) is a coherent discourse, and ([[poss-bath1]]) 'contains' an existential statement "there is a\(^v\) bathroom".
...
Implementing consistenct tests on the local context requires us to left EDS into an /update semantics/; epistemic modals need to be interpreted relative to an entire body of information, rather than a single evaluation point.
...
\(\neg\,\exists_vB(v) \vee \Diamond \neg\,(U(v))\)
#+end_exe

Note: not a Hurford's constraint violation, but if we spell out the local context of the second disjunct ([[disj2]]) we end up with an epistemic contradiction:
...
#+name: disj-contr 
#+begin_exe
\ex \(c[\neg\,\exists_vB(v)]^+ = \set{(w,g) \in c | I_w(B) = \emptyset }\)
\ex \(c[\neg\,\exists_vB(v)]^- = \set{(w,h)| (w,g) \in c \wedge g[v]h \wedge h_v \in I_w(B) }\)
\ex \(c[\Diamond\,\neg\,(U(v))]^+ = c\text{ if }\forall(w,g) \in c[g_v \neq \emptyset] \wedge \exists (w,g) \in c[g_v \in I_w(U)]\text{ else }\emptyset \)
#+end_exe

** Getting the truth-conditions right
...
What about presupposition projection? Well, epistemic modals are typically assumed to be holes, so:

#+name: epist3
#+begin_exe
\ex \(c[\Diamond\phi]^? = c[\phi]^?\)
#+end_exe
...
** On the anaphoric potential of modalized sentences

Consider the following minimal pair, instantiating a modal variant of Rothschild's observation.[fn:1]
...
- Check whether there is some point \((w,g) \in c\) at which \(\phi \) is true (consistency check).
- Update those points \((w,g) \in c\) with the information that \(\phi \), store the result as \(c'\).
- Update those points \((w,g) \in c\) with the information that \(\neg\,\phi \), store the result as \(c''\).
- Take return the union of the results.
...
\bigcup\limits_{(w,g) \in c}\set{(w,h)|(w,h) \in \set{(w,g)}[\phi]^{+,-,u}}&\exists (w,g) \in c[\set{(w,g)}[\phi]^+ \neq \emptyset]\\
\emptyset&\text{otherwise}  
\end{cases}\)
#+end_exe
...
Updating \(c_1\) with "Andreea might have a\(^v\) husband", first checks wheher any \((w,g) \in c_1\) can be updated to give a non-empty information state. Either \((w_a,g_\top )\) or \((w_b,g_\top )\) will satisfy this requirement. Since the consistency check is passed, we take the union of the following:

- \(\set{(w_a,g_\top)}[\exists_vH(v)]^+ = \set{(w_a,[v \to a])}\)
- \(\set{(w_a,g_\top)}[\exists_vH(v)]^- = \emptyset \)
- \(\set{(w_b,g_\top)}[\exists_vH(v)]^+ = \set{(w_b,[v \to b])}\)
- \(\set{(w_b,g_\top)}[\exists_vH(v)]^- = \emptyset \)
- \(\set{(w_\emptyset ,g_\top)}[\exists_vH(v)]^+ = \emptyset \)
- \(\set{(w_\emptyset ,g_\top)}[\exists_vH(v)]^- = \set{(w_\emptyset,g_\top)} \)
  
Resulting in the following updated file context:
