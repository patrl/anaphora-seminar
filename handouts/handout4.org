...
- Non-classicality (double negation elimination, de Morgan's laws).
- Anaphoric information flow is pre-compiled into the semantics of logical expressions.
...
- A general recipe (the ~State.Set~ applicative functor) for systematically lifting a static Montagovian fragment into a dynamic setting [cite:@Charlow2014].
